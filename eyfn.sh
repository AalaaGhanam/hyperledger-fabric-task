...
function networkUp () {
  ...
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to start Nbe network"
    exit 1
  fi
  echo
  echo "###############################################################"
  echo "############### Have Nbe peers join network ##################"
  echo "###############################################################"
  docker exec nbecli ./scripts/step2org3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to have Nbe peers join network"
    exit 1
  fi
  echo
  # finish by running the test
  docker exec nbecli ./scripts/testorg3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config ./org4-artifacts/crypto-config/ channel-artifacts/org3.json
...
function createConfigTx () {
  echo
  echo "###############################################################"
  echo "####### Generate and submit config tx to add Nbe #############"
  echo "###############################################################"
...
function generateCerts (){
  ...
  echo
  echo "###############################################################"
  echo "##### Generate Nbe certificates using cryptogen tool #########"
  echo "###############################################################"
...
function generateChannelArtifacts() {
  ...
  echo "##########################################################"
  echo "#########  Generating Nbe config material ###############"
  echo "##########################################################"
  (cd org4-artifacts
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg NbeMSP > ../channel-artifacts/org4.json
   res=$?
   set +x
   if [ $res -ne 0 ]; then
     echo "Failed to generate Nbe config material..."
     exit 1
   fi
  )
